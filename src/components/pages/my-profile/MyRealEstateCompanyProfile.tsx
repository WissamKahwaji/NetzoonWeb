...
const MyRealEstateCompanyProfile = ({
  userInfo,
}: MyRealEstateCompanyProfileProps) => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const userId = localStorage.getItem("userId");
  const [activeTab, ActiveTab] = useState("realestates");
  ...
  const tabs = [
    { tab: "realestates", label: "realestates" },
  ...
  } = useGetCompanyRealEstatesQuery(userId ?? "", activeTab === "realestates");
  const iconsmenu = [
    {
      text: "properties",
...
export default MyRealEstateCompanyProfile;
